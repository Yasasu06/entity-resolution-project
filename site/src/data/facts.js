...
export const PREDICTIONS = [
  ...
  { v: "held", claim: "Aggregate confidence is badly inflated",
    ...
    "The model's posteriors summed to 12,941 expected matches against 962 real ones — inflated 13.5×. The scores are not calibrated probabilities and were never treated as such." },
  { v: "failed", claim: "“A threshold will not fix it”",
    real: "56.8% → 75.0%", detail:
    "Precision rises 18 points when the threshold is raised. The reasoning came from planted probes, which measured only near-miss confusion — genuinely threshold-invariant. Most wrong accepts fail for a different reason: the record has no partner at all. The decision to keep the threshold still holds on F1; the reasoning did not." },
...
export const STEPS = [
  { k: "Method one", h: "An independent judge",
    p: "A language model decided 704 of the auto-accepted pairs cold, never seeing a score. It disagreed with 31%. Disagreement fell steadily with score — 72.6% in the lowest band, 11.2% in the highest — the signature of real signal rather than a capricious judge." },
